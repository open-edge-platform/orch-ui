...
export const handlers = [
  rest.get(`${baseURL}/alerts/definitions/:alertDefinitionID`, (req, res, ctx) => {
    const { alertDefinitionID } = req.params as { alertDefinitionID: string };
    const alert = alertDefinitionStore.get(alertDefinitionID);
    if (!alert) {
      return res(ctx.status(404), ctx.json({ error: "Alert definition not found" }));
    }
    return res(ctx.status(200), ctx.json(alert));
  }),


  rest.put(`${baseURL}/alerts/definitions/:alertDefinitionID`, async (req, res, ctx) => {
    const { alertDefinitionID } = req.params as { alertDefinitionID: string };
