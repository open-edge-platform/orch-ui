...
{status: 200,
  ...
  http.get(`/v2/projects/${projectName}/clusters/:name`, () => {
    const { name } =
...
{status: 404,
 ...
 let req = request;
    const body = await req.json<any, cm.ClusterSpec>();
    const r = cs.post(body);
    if (!r) return HttpResponse.text(
{status: 500,
});
    return HttpResponse.text(
{status: 200,
...
{status: 200,
 ...
 let req = request;
      const { name } =
        params as cm.GetV2ProjectsByProjectNameClustersAndNameApiArg;
      const body = await req.json<any, cm.ClusterTemplateInfo>();
...
{status: 200,
 ...
 let req = request;
      const { name } =
        params as cm.GetV2ProjectsByProjectNameClustersAndNameApiArg;
      const body = await req.json<any, cm.ClusterLabels>();
      const r = cs.put(name, body);
      if (!r) return HttpResponse.text(
{status: 500,
});
...
{status: 404,
  ...
  //delete cluster
  http.delete(`/v2/projects/${projectName}/clusters/:name`, () => {
    const { name } =
...
{status: 501,
 ...
 let req = request;
      ...
      const nodeSpecs = await req.json<any, cm.NodeSpec[]>();
