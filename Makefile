...
# Working MFE admin|eim|cluster-orch|app-orch
APP						?=
# Configure shell
...
# Variables
VERSION                 ?= $(shell python3 -c 'import json,sys;obj=json.load(sys.stdin); print(obj["version"])' < package.json)
DOCKER_TAG              ?= $(VERSION)
VERSION_DEV_SUFFIX      :=
...
HELM_CHART = "apps/$(APP)/deploy/"
IS_RELEASED_VERSION=$(shell MY_STRING="${VERSION}"; MY_REGEX='^([0-9]+)\.([0-9]+)\.([0-9]+)$$'; if [[ $$MY_STRING =~ $$MY_REGEX ]]; then echo true; else echo false; fi)

admin_version = $(shell python3 -c 'import json,sys;obj=json.load(sys.stdin); print(obj["version"])' < ./package.json)
...
docker-build: dist ## @HELP Build the orch-ui docker image
...
helm-version-check-all: helm-version-check

helm-clean: helm-reset-annotations ## @HELP Clean helm chart build annotations.
	rm app/${APP}/orch-ui-admin-*.tgz || true
...
helm-push: ## @HELP Push helm charts.
	# publish Admin MFEs charts
	helm push orch-ui-admin-${VERSION}.tgz $(HELM_REGISTRY)/$(HELM_REPOSITORY) --debug
...
apply-version: helm-clean ## @HELP apply version from the top level package.json to all sub-projectsare the same across the different projects
	...
	for d in $(HELM_DIRS); do \
		yq eval -i '.version = "${VERSION}"' $$d/Chart.yaml ; \
		yq eval -i '.appVersion = "${VERSION}"' $$d/Chart.yaml ; \
	done
	yq eval -i '.dependencies[].version = "${VERSION}"' apps/${APP}/deploy/Chart.yaml
